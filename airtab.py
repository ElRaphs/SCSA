...
class AirmetTab(FunctionsAM):
    def airtab(self):
        ...
        self.air_create_msg_button = Button(self.airframe, text='Criar AIRMETs', command=lambda: self.air_check_error())
        ...
        self.air_embd_ts = Radiobutton(self.frame_2, text='ISOL TS', background='#009DE0', variable=self.air_type, value='ISOL TS', command=self.air_ts_true)
        self.air_embd_ts.place(relx=0.01, y=120)
        self.air_obsc_ts = Radiobutton(self.frame_2, text='OCNL TS', background='#009DE0', variable=self.air_type, value='OCNL TS', command=self.air_ts_true)
        self.air_obsc_ts.place(relx=0.01, y=150)

        #---------------------------------INPUT SFC VIS-----------------------------------------------
        self.air_lb_vis = Label(self.frame_2, text='VIS', background='#009DE0')
        self.air_vis_lb = Radiobutton(self.frame_2, text='SFC VIS', background='#009DE0', variable=self.air_type, value='SFC VIS')
